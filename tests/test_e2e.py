...
class TestEndToEndFlow:
    ...
    async def test_complete_user_flow(self, tmp_memory_dir, mock_embedder):
        ...
        for content in contents:
            ...
            await l4_store.save(memory_id, content, {
                "importance": 0.8,
                "category_path": category
            })

        # Step 2: List 查看
        all_ids = l4_store.list()
        assert len(all_ids) >= 3, "应该有至少3条记忆"

        # Step 3: Load 查看内容
        for memory_id in all_ids[:3]:
            content = await l4_store.load(memory_id)
        ...
        if all_ids:
            delete_id = all_ids[0]
            await l4_store.delete(delete_id)

        # Step 5: 验证剩余
        remaining = l4_store.list()
    ...
    async def test_two_file_group_generation(self, tmp_memory_dir):
        ...
        memory_id = generate_mem_id()
        content = "测试内容"
        await l4_store.save(memory_id, content, {"importance": 0.8})
    ...
    async def test_metadata_integrity(self, tmp_memory_dir):
        ...
        await store.save(memory_id, "测试记忆", metadata)
    ...
    async def test_unicode_content_persistence(self, tmp_memory_dir):
        ...
        memory_id = generate_mem_id()
        await l4_store.save(memory_id, special_content, {"importance": 0.8})

        # 验证读取
        loaded = await l4_store.load(memory_id)
    ...
    async def test_update_flow(self, tmp_memory_dir):
        ...
        memory_id = generate_mem_id()
        await l4_store.save(memory_id, "原始内容", {"importance": 0.5})

        # 更新元数据（load + re-save）
        data = await l4_store.load_existing(memory_id)
        meta = data["meta"]
        meta["importance"] = 0.9
        meta["tags"] = ["更新过"]
        await l4_store.save(memory_id, data["content"], meta)

        # 验证更新
        updated = await l4_store.load_existing(memory_id)
        assert updated["meta"]["importance"] == 0.9
        assert updated["meta"]["tags"] == ["更新过"]
