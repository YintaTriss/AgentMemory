...
- **最少 3 层**：对应图书馆三级结构（**馆分类 / 书架分类 / 书分类**）
  - 第 1 层（馆分类）：5 个固定顶层类之一
  - 第 2 层（书架分类）：按领域/主题细分
  - 第 3 层及以下（书分类）：按具体事项/子项目
- **最大不设限**：`max_depth=None`，动态层数，按需延伸
- 第 1 层必须是上述 5 个固定类之一。
- 第 2 层及以下可自由命名（建议中文，slug 化）。

**有效路径示例**：

```
项目/石榴籽/语料/NLLB训练                 ✅ 4 层
项目/石榴籽/语料/NLLB训练/2026-06         ✅ 5 层（可继续延伸）
学习/AI/Transformer                        ✅ 3 层
偏好/Pythonic                               ❌ 2 层（最少 3 层）
```
