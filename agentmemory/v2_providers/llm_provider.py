...
提供多种 LLM 实现：
- MockLLM: 固定响应，用于测试/离线环境
- OpenAILLM: OpenAI ChatGPT API
- BailianLLM: 阿里云百炼 Qwen API
- MiniMaxLLM: MiniMax API
"""
...
class BailianLLM:
    ...
    async def chat(self, messages: list[dict], **kwargs) -> LLMResponse:
        ...
        response = await client.post(
            f"{self.base_url}/chat/completions",
            json=payload,
            headers={"Authorization": f"Bearer {self.api_key}", "Content-Type": "applicatio
