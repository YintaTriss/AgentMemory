...
'''
    content = content.replace(search_pattern, new_methods)
    print('Added RRF methods after search_hybrid')
else:
    print('WARNING: Could not find search_hybrid end marker')
    # Debug: show what's around the return results line
    idx = content.find('        return results')
    if idx >= 0:
        print('Found "return results" at', idx)
