...
| **L3** | `L3QdrantStore` | Qdrant Edgeベクトル検索（利用不可時は自動的にPure JSON + numpyにフォールバック）、BM25ハイブリッド検索対応 |
...
| `L3QdrantStore` | `l3_qdrant.py` | Qdrant Edgeベクトル検索 + JSON Fallback + BM25ハイブリッド検索 |
...
| **Qdrant Edgeインジェクション保護** | `web.py` / `cli.py` | category_pathの単一引用符を`''`にエスケープ（SQL標準）|
...
pip install agentmemory[qdrant] # Qdrant Edgeベクトルデータベース（高性能シナリオ）
pip install agentmemory[dev]     # 開発依存（pytestなど）
```

> Qdrantが利用できない場合（未インストール）、システムは自動的にPure JSON + numpyにフォールバック—追加依存なしで実行可能。
...
| `AGENT_MEMORY_DATA_DIR` | `data` | ベクトルデータディレクトリ（Qdrant Edge / JSON Fallback） |
...
| Qdrant Edge優先 + JSON Fallback | Qdrant利用不可時に自動デグレード | 高性能シナリオはQdrant Edge、依存ゼロシナリオはJSON |
